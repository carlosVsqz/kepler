package com.escod.kepler.serialization;
...
import io.jmix.core.impl.serialization.EntityAttributeSerializationExtensionResolver;
...
@Primary
@Component("coreEntitySerializationKepler")
public class CoreEntitySerialization implements EntitySerialization {

  private static final Logger log = LoggerFactory.getLogger(CoreEntitySerialization.class);
  ...
  @Autowired
  protected StorageKeplerService storageKeplerService;
      ...
      for (MetaProperty metaProperty : properties) {
        ...
        if (propertyRange.isDatatype()) {
          if (fieldValue instanceof Collection) {
            ...
            // todo map to download url and validate if request from rest
            if (fieldValue instanceof FileRef fileRef) {
              List<String> mediaTypes = Arrays.asList(MediaType.IMAGE_PNG_VALUE, MediaType.IMAGE_JPEG_VALUE, MediaType.IMAGE_GIF_VALUE);
              String mediaType = fileRef.getContentType();
              if (mediaTypes.contains(mediaType)) {

                // replace 4 storage url
//                String value = "https://i.ibb.co/RDhZKFc/Captura-de-pantalla-2024-02-16-11-43-53.png";
//                writeSimpleProperty(jsonObject, value, metaProperty);
                fieldValue = storageKeplerService.getURLPublicPassFromRef(fileRef);
              }
            }
